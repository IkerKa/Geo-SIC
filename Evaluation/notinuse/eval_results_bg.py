...
# Interpolación de la imagen (igual que MATLAB interp3)
points = (np.arange(X.shape[0]), np.arange(X.shape[1]), np.arange(X.shape[2]))
wwarped = interpn(
    points,
    nirep01,
    np.stack((XI.ravel(), YI.ravel(), ZI.ravel()), axis=-1),
    method='linear',
    bounds_error=False,
    fill_value=0
)
...
# --- Asegurar orden de ejes igual a MATLAB ---
# MATLAB: [Y, X, Z], Python: [X, Y, Z] por defecto
# Si tus segmentaciones no coinciden visualmente con MATLAB, descomenta la siguiente línea:
# seg = np.transpose(seg, (1, 0, 2))
# reference = np.transpose(reference, (1, 0, 2))

# --- Asegurar tipo entero para comparación de etiquetas ---
...
# --- Interpolación nearest para segmentación (igual que MATLAB interp3 'nearest') ---
coords = np.stack([
    iphi[..., 0].ravel(),
    iphi[..., 1].ravel(),
    iphi[..., 2].ravel()
...
# Redondear como MATLAB interp3 'nearest'
coords_rounded = np.round(coords).astype(int)
for i in range(3):
    coords_rounded[:, i] = np.clip(coords_rounded[:, i], 0, dim_s[i] - 1)

# Indexar directamente (más rápido y exacto que interpolador nearest)
seg_w = seg[coords_rounded[:, 0], coords_rounded[:, 1], coords_rounded[:, 2]].reshape(dim_s)
print("seg_w shape:", seg_w.shape, "unique labels:", np.unique(seg_w))

for i, name in enumerate(['X', 'Y', 'Z']):
    out_of_bounds = np.sum((coords_rounded[:, i] < 0) | (coords_rounded[:, i] >= dim_s[i]))
    print(f"{name} out of bounds: {out_of_bounds}")
